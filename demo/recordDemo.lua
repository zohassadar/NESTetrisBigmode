...
function registerListeners() 
  ...
  memory.registerexecute(0x9C06, handleButtons);  -- lda heldButton in @recording right after ldx 00 and before sta (demoButtons,X)
  memory.registerexecute(0x9C0D, handleRepeats);  -- lda repeats in @recording after jsr and before sta
end

function unregisterListeners()
  memory.register(0x00D0, nil)
  memory.registerexecute(0x9C06, nil)
  memory.registerexecute(0x9C0D, nil)
end
...
function handleButtons() 
  ...
  memory.setregister("pc", 0x9C08);   -- First occurence of sta (demoButtonsTable,X)
end

function handleRepeats() 
  bytes = bytes .. string.format("%02X ", memory.readbyte(0x00CF))
  memory.setregister("pc", 0x9C0F);    -- Second occurance
end
